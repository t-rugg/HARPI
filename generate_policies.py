# BANNED MODIFIERS: dev cost, core cost, diplo annex cost, admin eff, anything related to gov cap, province warscore cost, innovativeness gain, idea cost, tech cost, accept vassal reasons
...
adm = {
	...
	"humanist_ideas": ("global_unrest  = -1", "improve_relations = 0.15", "land_leader_maneuver = 1", "stability_cost_modifier = -0.15", "sailors_recovery_speed = 0.15", "morale_damage_received = -0.075"),
...
dip = {
	"spy_ideas": ("max_absolutism = 5", "dip_advisor_cost = -0.2", "leader_siege = 1", "interest = -0.05", "global_foreign_trade_power = 0.15", "harsh_treatment_cost = -0.25"),
	"diplomatic_ideas": (),
	"trade_ideas": (),
	"exploration_ideas": (),
	"maritime_ideas": (),
...
mil = {
	"offensive_ideas": ("loot_amount = 0.5", "improve_relations = 0.15", "shock_damage = 0.075", "years_of_nationalism = -5", "trade_company_investment_cost = -0.1", "artillery_barrage_cost = -0.25"),
}
...
with open("generated_policies.txt", "w", encoding="windows-1252") as f: 
	...
	for group1 in ideas:
		...
		for group2 in [key for key in ideas if key not in blacklist]:
			...
			if (cat1 != cat2):
				...
				formatted = formatter.format(group1, group2, cat, mainMod, subMod)
